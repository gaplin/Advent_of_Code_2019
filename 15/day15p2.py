...
def get_all_tiles(mem: list) -> (set, tuple):
    position = (0, 0)
    Q = Queue()
    ...
    return (all_nodes, oxygen_position)
    
def get_longest_path(start, all_nodes):
    Q = Queue()
    Q.put((start, 0))
    visited = {start}
    result = 0
    ...
    while Q.empty() == False:
        ...
        for di, dii in directions:
            new_position = (position[0] + di, position[1] + dii)
            if new_position not in visited and new_position in all_nodes:
                visited.add(new_position)
                Q.put((new_position, distance + 1))

    return result

all_nodes, oxygen_position = get_all_tiles(mem)
result = get_longest_path(oxygen_position, all_nodes)
print(result)